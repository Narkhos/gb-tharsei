...
void draw_labyrinthe(unsigned int x, unsigned int y) {
	if (english) {
		text_print_string_bkg(x, y,      "I WAS IN A LABYRINTH");
		text_print_string_bkg(x, y +  1, "OF CORRIDORS.       ");
...
UINT8 state_labyrinthe() {
	...
	if (english) {
		text_print_string_win(0, 1, "GO");

		text_print_string_win(1, 2, "NORTH");
		text_print_string_win(1, 3, "SOUTH");
		text_print_string_win(1, 4, "EAST");
		text_print_string_win(1, 5, "WEST");
	} else {
